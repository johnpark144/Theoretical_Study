...
- vue와 리액트의 차이점
  - vue 
    ...
    - 컴포넌트 <a href="">분리 및 재사용</a> 측면에서 <a href="">효율성이 낮음</a>
    - 리액트보다 <a href="">낮은 생태계</a> 구성
  - 리액트
    - <a href="">라이브러리</a>
    ...
    - 컴포넌트 <a href="">분리 및 재사용</a> 측면에서 <a href="">효율성이 높음</a>
    - vue보다 <a href="">높은 생태계</a> 구성
...
- 리액트에서 함수형 컴포넌트라고 부르지 않고 함수 컴포넌트라고 부르는 이유가 무엇인가요🔥
  - 초창기에는 함수형 컴포넌트라 불렸지만 함수형 <a href="">프로그래밍과 이름이 비슷</a>하여 혼란스러울 수 있기 때문에 최근에 <a href="">함수 컴포넌트로 이름이 바뀜</a>
...
- SPA가 뭔가요?
  ...
  - 한 페이지로 필요한 부분만 자바스크립트를 사용하여 업데이트 해주고, 서버 API를 호출하여 필요한 데이터만 새로 불러와 사용할 수도 있습니다
  - 리액트는 리액트 라우터, NextJS등으로 라우팅을 구현
  - SPA의 단점은 SEO에 취약하다(검색엔진에 노출되기 힘듬)

- SSR이 뭔가요?
  - 서버로부터 완전히 만들어진 html파일을 받아와 페이지 전체를 렌더링 하는 방식
  - SEO에 최적화 (검색엔진들이 html태그들을 분석하는데 CSR는 빈 html이고 SSR은 만들어진 html임)
  - 첫 페이지 로딩속도가 빠르다
  - 서버 측에서 모든 뷰를 준비하면(MPA) 과부하로 성능상의 문제가 발생할 수 있어서 느려지기 쉬우며, 페이지 마다 이동시 깜박이는 현상이 발생됨.
  - 처음 화면이 보이기 시작해도(TTV) 상호작용(TTI)되기 전까지 기다려야함 (JS파일 다운시간)

- SEO가 뭔가요?
  - 검색엔진 최적화 : 서버에 있는 HTML 파일을 분석하여서, 검색어를 찾을때 데이터를 크롤링하여 웹사이트에 포함 되있으면 그 웹페이지를 보여주게 함
  - SSR, Meta태그 등을 이용하면 도움됨

- TTV, TTI
  - TTV : Time To View (볼 수 있는 시점)
  - TTI : Time To Interact (인터랙션 할 수 있는 시점)
  - CSR의 경우 TTV와 TTI가 같음 [접속 -> 빈 html파일 받아옴 -> JS파일 받아옴 -> JS 작동시(TTI) 동적으로 html생성(TTV)]
  - SSR의 경우 TTV가 먼저 TTI가 그 다음 [접속 -> 완성된 html파일 받아옴(TTV) 그러나 클릭해도 아무처리 안됨 -> JS파일 받아옴 -> JS 작동(TTI)]
  - 개발시 주의 점
    - CSR의 경우 어떻게 하면 효율적으로 많이 분할하여 첫 번째로 사용자가 보기 위해서 필요한 정말 필수적인 html 요소만 보낼 수 있을지 고민
    - SSR의 경우 인터렉션(TTI)하는 이 시간의 단차를 줄이기 위해서 어떤 노력을 할 수 있을지 고민, 어떻게 매끄럽게 UX/UI를 제공할 수 있을지 고민
- SSG란?
  - 미리 컴파일된 정적인 HTML을 생성하여 서버에 배포하는 방식 (미리 다 만들어져있는 페이지)
  - 업데이트가 자주 일어나지 않는 사이트에 적합
  - 장점 : 빠른 로딩 속도, SEO, 보안 (서버X, 데이터베이스X)
  - 단점 : 정적 파일들을 수정 및 추가하려면 새로운 빌드 해야함

- 하이드레이션에 대해 알고 있나요
  - SSR 혹은 SSG로 만들어진 정적인 HTML과 State를 수분이 없다 표현하고, 이로부터 동적인 상태로 변화하는 것이 수분을 보충하는 과정과 같아서 하이드레이션이 일어난다고 표현함.
  - render()는 빈 HTML로 UI를 생성하거나 변경사항을 업데이트하는것, hydration()은 이미 만들어진 HTML에 리액트 트리에 맞게 이벤트를 보충하는 것
