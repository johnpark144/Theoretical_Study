- `프로세스와 스레드 🔥`
  - 프로세스와 스레드의 차이
    - 프로세스 : 프로그램의 동적상태가 될때 작업의 단위, 독자적으로 시스템 자원을 할당 받음
    - 스레드 :  할당받은 자원을 이용하는 실행 흐름의 단위, 메모리 영역을 다른 스레드와 공유

- `싱글 스레드와 멀티 스레드 🔥`
  - 싱글 스레드 장단점
    - 장점 : 스레드 간의 작업전환 X -> 단순 작업엔 유리
    - 단점 : 여러 CPU를 활용 X -> 동시 작업 하는 느낌 X
  - 멀티 스레드 장단점
    - 장점 : 1) 새로운 프로세스보다 기존 프로세스에서 스레드 생성이 더 빠름, 2) 프로세스의 자원과 상태를 공유하여 효율적, 3) 프로세스 작업전환보다 스레드 작업전환이 더 빠름.
    - 단점 : 1) 단순작업엔 오히려 시간을 지연, 2) 스레드 스케쥴링을 신경써야 한다.

- `HTTP 🔥`
  - HTTP란 (HyperText Transfer Protocol) ? 웹과 서버간의 데이터를 주고 받기 위한 통신 프로토콜 (FTP는 파일전송용)
  - HTTP 프로토콜의 가장 큰 특징 : 현 데이터 요청과 다음 데이터 요청이 서로 관련이 없음 (state가 없음)

  - URL (Uniform Resource Locators) 이란? 
    - 자원 요청을 위한 영문 주소
    - 숫자보다 쉬움
    - DNS를 통해 실제 IP 주소로 변환

  - DNS의 장점
    - IP 주소가 바뀌어도 똑같은 도메인 주소로 사용가능

  - HTTP/1.1 과 HTTP/2.0의 차이
    - HTTP/1.1 : 커넥션당 하나의 요청/응답, 응답 시간이 느림
    - HTTP/2.0 : 커넥션당 다중 요청/응답, 응답 시간이 빨라짐

  - HTTPS와 HTTP
    - HTTPS의 S는 secure을 의미
    - 내가 사이트에 보내는 정보를 암호화시킴
    - 이로인해 믿을만한 

  - 대칭키와 비대칭키(공개키와 개인키)
    - 대칭키는 똑같은키로 암호화 및 복호화
    - 비대칭키는 짝된키로 공개키와 개인키이다, 공개키로 암호화 개인키로 복호화
    - 공개키를 공유할 때는 주로 대칭키 방식을 사용
...
- `쿠키 세션 🔥`
  - 쿠키, 세션 쓰는 이유 🔥🔥
    - HTTP는 비연결성이라는 특징이 있지만 이들로 인해 전에 기록이 있으면 재연결되도록 함.

  - 쿠키란? 🔥🔥
    - 웹 사이트를 방문한 경우, 클라이언트에 저장되는 기록 정보이다.
    - 클라이언트가 갖고있음
    - 보안에 취약하여 중요정보를 담지않음
  ...
  - 세션이란? 🔥🔥
    - 서버가 클라이언트를 식별할 수 있도록 하는 방식
    - 서버가 갖고있음
    - 쿠키를 이용하여 클라이언트에 세션 ID 값을 부여 (웹브라우저 종료시 세션쿠키 삭제)
...
- `CORS 🔥`
  - CORS란 
      ...
      - SOP(Same Origin Policy)인 동일 출처 정책에 의해 똑같은 URL끼리만 데이터접근 가능하게 하지만 CORS를 허용하면 그것을 풀어주는 역할을 함(화이트리스트).
...
- `클라우딩 서비스 (off-premise 방식)`
  ...
  - PaaS : (기본빌트인)
      ...
      - 운영비 절감효과
  - SaaS 🔥 : (완성)
      - 서비스로서의 소프트웨어(Software-as-a-Service, SaaS)는 완성된 클라우드 컴퓨팅 형태
      - SaaS의 예로는 Google Docs, 인사 관리 소프트웨어, 고객 관계 관리 툴, IDE(통합 개발 환경) 등
...
- `개발 방법론 🔥`
  - 폭포수 방법론 (순서중심)
      ...
      - 전 단계 끝나기전 다음으로 X
  - 애자일 방법론 (CI/CD 중심)
      - 변화에 대응하며, 상호작용 및 만족도 개선을 위해 반복작업하는 개발 방식

- `Cache 🔥`
  - 캐시란?
      - 자주 필요한 데이터나 반복적인 결과를 일시적으로 저장하여 전체적인 처리 속도를 향상
      - CPU가 메모리를 기다라는 병목현상을 완화하기 위해 속도 빠른 캐시 메모리를 둠
- `CI/CD 🔥`
  - CI/CD란?
      - 개발 단계부터 빌드까지 지속적으로 작은단위로 빈번하게 배포하는 방법
...
- `CDN 🔥`
  - CDN(엣지/콘텐츠 전송 네트워크)이란?
      - 지리적으로 분산된 여러 개의 서버
      - 사용자가 요청하면 가까운 곳에서 응답 (지연시간 줄임)
      - 과부화 방지
      - 오히려 대역폭 비용이 절감 (한 서버에 대역폭을 늘릴필요X)
      - 하나가 오류 생겨도 다른 CDN으로 연결되기 떄문에 안정적

- `테스트(TDD) 🔥`
  - 테스트란? 어플리케이션이 요구사항에 맞게 동작하는지를 검증하는 행위
  - 테스트의 예
     - 버그 수정 및 리팩토링 후 영향받는 다른 모듈의 실행 결과 확인
  ...
  - 유닛 테스트란?
     - 모듈(함수,클래스) 테스트
     - 작성 비용이 적고, 실행속도가 빠름
     - 문제가 생긴 부분을 비교적 쉽고 정확하게 파악
  - 통합 테스트란?
     - 단위 테스트보다 큰 범위
     - 개별 모듈(함수/클래스)들이 연결되어 제대로 상호작용 하는지를 테스트
